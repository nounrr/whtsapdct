...
async function getLogsStats(whatsappClient = null) {
  try {
    ...
    // Calculer le nombre total de messages envoyés (succès)
    ...
    if (whatsappClient) {
      try {
        ...
        let oldMessagesSent = 0;
        let oldMessagesSentToday = 0;

        // Compter les messages envoyés dans tous les chats
        for (const chat of chats) {
          try {
            // Récupérer les messages du chat (limité pour performance)
            const messages = await chat.fetchMessages({ limit: 1000 });
            
            // Filtrer les messages envoyés par nous (fromMe = true)
            const sentByMe = messages.filter(msg => msg.fromMe);
            oldMessagesSent += sentByMe.length;
            ...
            oldMessagesSentToday += todaySent.length;
          } catch (chatErr) {
            console.warn(`[logger] Erreur lecture chat ${chat.id._serialized}:`, chatErr.message);
          }
        }

        stats.oldMessagesSent = oldMessagesSent;
        stats.totalMessagesAllTime = totalMessagesSent + oldMessagesSent;
        stats.oldMessagesSentToday = oldMessagesSentToday;
        stats.totalMessagesSentToday = todayMessagesSent + oldMessagesSentToday;
